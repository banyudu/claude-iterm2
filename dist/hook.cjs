...
function scheduleTimer(project) {
...
function writeHeartbeat() {
  const sessionId = getSessionId();
  ensureStateDir();
  import_node_fs4.default.writeFileSync(heartbeatFilePath(sessionId), String(Math.floor(Date.now() / 1e3)));
}
function claudeSessionFilePath(sessionId) {
  return import_node_path3.default.join(gradientStateDir, `${sessionId}.claude_session`);
}
function writeClaudeSessionId(claudeSessionId) {
  const sessionId = getSessionId();
  ensureStateDir();
  import_node_fs4.default.writeFileSync(claudeSessionFilePath(sessionId), claudeSessionId);
}
...
function startWorkingWatchdog(project) {
  const sessionId = getSessionId();
  ensureStateDir();
  stopWorkingWatchdog();
...
// src/status.ts
function applyStatus(status, project) {
...
async function main() {
  ...
  const project = import_node_path5.default.basename(process.cwd());
